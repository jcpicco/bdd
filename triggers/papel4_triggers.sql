...
BEFORE INSERT OR UPDATE ON PAPEL4.HOTEL48 FOR EACH ROW
DECLARE
    ECOUNT NUMBER;
BEGIN
    IF(:OLD.EID != :NEW.EID) THEN
        ...
        IF (ECOUNT = 0) THEN
            RAISE_APPLICATION_ERROR(-20094,'Este empleado no existe');
        END IF;
    END IF;
END;
/
...
BEFORE INSERT ON PAPEL4.HOTEL48 FOR EACH ROW
DECLARE
    HCOUNT NUMBER;
    ECOUNT NUMBER;
BEGIN
    ...
    IF (HCOUNT > 0) THEN
        RAISE_APPLICATION_ERROR(-20095,'El hotel ya existe');
    END IF;
    ...
    IF (ECOUNT > 0) THEN
        RAISE_APPLICATION_ERROR(-20096,'Este empleado ya dirige un hotel.');
    END IF;
END;
/
...
BEFORE INSERT ON PAPEL4.EMPLEADO48 FOR EACH ROW
BEGIN
    IF(:NEW.FECHA_CONTRATO > :NEW.FECHA_INI) THEN
        RAISE_APPLICATION_ERROR(-20097,'Fecha de contrato mayor a la de inicio');
    END IF;
END;
/
...
BEFORE UPDATE ON PAPEL4.EMPLEADO48 FOR EACH ROW
BEGIN
    IF(:NEW.SALARIO < :OLD.SALARIO) THEN
        RAISE_APPLICATION_ERROR(-20098,'El salario no puede disminuir.');
    END IF;
END;
/
...
BEFORE INSERT OR UPDATE ON PAPEL4.RESERVA48 FOR EACH ROW
DECLARE
    NUM_HABS NUMBER;
BEGIN
    IF :NEW.FECHA_FIN < :NEW.FECHA_INI THEN
        RAISE_APPLICATION_ERROR(-20099,'Fecha de salida no puede ser menor a la de entrada');
    END IF;
END;
/
...
BEFORE INSERT ON PAPEL4.RESERVA48 FOR EACH ROW
DECLARE
...
BEGIN
    ...
    IF(RESERVAS > 0) THEN
        RAISE_APPLICATION_ERROR(-20100,'El cliente ya tiene una reserva en esas fechas.');
    END IF;
    ...
    ELSIF (:NEW.TIPO_HAB = 'Doble') THEN
        ...
        WHERE HID = :NEW.HID;
    ELSE
        RAISE_APPLICATION_ERROR(-20101,'Tipo de habitación no válida.');
    END IF;
    ...
    IF(RESERVASHOTEL = NHABITACIONES) THEN
        RAISE_APPLICATION_ERROR(-200102,'No queda espacio en el hotel en esas fechas.');
    END IF;
END;
/
...
BEFORE INSERT ON PAPEL4.PROVEEDOR1 FOR EACH ROW
DECLARE
PCOUNT NUMBER;
BEGIN
    SELECT COUNT(*)
    ...
    IF(PCOUNT > 0) THEN
        RAISE_APPLICATION_ERROR(-20103,'Proveedor existente.');
    END IF;
END;
/
...
BEFORE DELETE ON PAPEL4.PROVEEDOR1 FOR EACH ROW 
DECLARE
...
BEGIN
	...
	IF(PROVEEDOR_ACTIVO > 0) THEN
		RAISE_APPLICATION_ERROR(-20104,'Este proveedor no puede eliminarse, sigue activo');
    END IF;
END;
/
...
BEFORE INSERT ON PAPEL4.ARTICULO FOR EACH ROW 
BEGIN
	IF(:NEW.TIPO != 'A' AND :NEW.TIPO != 'B' AND :NEW.TIPO != 'C' AND :NEW.TIPO != 'D') THEN
		RAISE_APPLICATION_ERROR(-20105,'El tipo de articulo introducido no se admite en la base de datos.');
    END IF;
END;
/
...
BEGIN
    ...
    IF (ACOUNT = 1) THEN
        ...
        IF(LOCALIDAD = LOCALIDADNUEVO) THEN
            RAISE_APPLICATION_ERROR(-20106,'Los artículos los suministra un solo proveedor por provincia.');
        END IF;
    ELSIF(ACOUNT > 1) THEN
        RAISE_APPLICATION_ERROR(-20107,'Ya hay un proveedor por provincia que suministra el artículo.');
    END IF;
END;
/
...
BEFORE DELETE ON PAPEL4.ARTICULO FOR EACH ROW 
DECLARE
...
BEGIN
	...
	IF(ARTICULO_ACTIVO != 0) THEN
		RAISE_APPLICATION_ERROR(-20108,'Este articulo no puede eliminarse, sigue activo');
    END IF;
END;
/
...
BEFORE INSERT ON PAPEL4.ENTREGA48 FOR EACH ROW
DECLARE
    PRECIO_AUX NUMBER;
BEGIN
    BEGIN
    ...
    IF(:NEW.PRECIO < PRECIO_AUX) THEN
        RAISE_APPLICATION_ERROR(-20109,'El precio del artículo del suministro no puede ser inferior.');
    END IF;
END;
/
...
BEGIN
	...
	IF(PROVEEDOR_SEVILLA != 0) THEN
		RAISE_APPLICATION_ERROR(-20110,'Granada, Jaén, Málaga y Almería, no pueden solicitar este proveedor');
    END IF;
END;
/